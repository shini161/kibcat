...
from url_jsonifier.parsers import parse_rison_url_to_json
import json
import os
...
KIBANA_URL = (
        ...
        "dataSource:(dataViewId:'container-log*',type:dataView),"
        "filters:!(('$state':(store:appState),"
        "meta:(alias:!n,disabled:!f,field:kubernetes.namespace,index:'container-log*',"
        "key:kubernetes.namespace,negate:!f,params:(query:qa),type:phrase),"
        "query:(match_phrase:(kubernetes.namespace:qa)))),"
        "grid:(columns:('@timestamp':(width:127),agent.id:(width:159),"
        "cometa.log.message:(width:249),kubernetes.namespace:(width:202))),"
        "interval:auto,query:(language:kuery,query:'kubernetes.pod.name%20:%20%22backend%22'),"
        "sort:!(!('@timestamp',desc)))"
        )
